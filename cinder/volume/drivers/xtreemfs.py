...
class XtreemfsDriver(nfs.RemoteFsDriver):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._mount_cmd = 'mount.xtreemfs'
    ...
    def do_setup(self, context):
        ...
        try:
            self._execute(self._mount_cmd, check_exit_code=False,
                          run_as_root=True)
        except OSError as exc:
            if exc.errno == errno.ENOENT:
                msg = _('%s is not installed') % self._mount_cmd
                LOG.error(msg)
                raise exception.XtreemfsException(msg)
            else:
                raise
    ...
    def _mount_xtreemfs(self, xtreemfs_share, mount_path):
        ...
        self._execute(self._mount_cmd, '-o', 'allow_other', '--max-tries', 3,
